...
const getSingleInventory = async (req, res) => {
  try {
    ...
    res.status(500).send(`Error in getting single Inventory:getSingleInventory() method: ${error}`);
  }
}
...
const editSingleInventory = async (req, res) => {
  try {
    ...
    if (!req.body.id || !req.body.warehouse_id || !req.body.item_name ||
      !req.body.description || !req.body.category || !req.body.status || !req.body.quantity) {
      return res.status(400).send("Please provide all information for the request");
    }

    //Response returns 400 if the quantity is not a number
    if (!(typeof req.body.quantity === 'number')) {
      return res.status(400).send(`Quantity is not a number for inventory id ${req.body.id}`);
    }
    ...
    const inventoryFound = await knex('inventories').where({ id: req.params.id });
    if (!inventoryFound[0]) {
      return res.status(404).send(`The inventory_id ${req.params.id} does not exist in the inventories table`);
    }
